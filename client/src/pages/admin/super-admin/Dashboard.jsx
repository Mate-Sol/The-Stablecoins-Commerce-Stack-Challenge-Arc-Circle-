...
import { CreditCard, Users, FileCheck, AlertTriangle, LogOut, Eye, Loader2, Search, DollarSign, Clock, BarChart2, ChevronLeft, ChevronRight } from 'lucide-react';
...
const SuperAdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Always fetch stats and alerts on first load or refresh
      if (page === 1) {
      ...
      if (activeTab === 'applications') {
        const appsResponse = await adminAPI.getApplications({ search, page, limit });
        setApplications(appsResponse.data.applications || []);
        setTotalPages(appsResponse.data.totalPages || 1);
      } else {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="ml-64 p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <AlertsBanner
            overdue={alerts.overdueList}
            dueSoon={alerts.dueSoonList}
            isAdmin={true}
          />
          ...
          <div className="table-container mb-8">
            <div className="px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <h2 className="text-lg font-semibold whitespace-nowrap">
                {activeTab === 'applications' ? 'PSP Profile Applications' : 'Pending Financing Requests'}
              </h2>
            ...
            {loading ? (
              ...
              <div className="p-12 text-center">
                {activeTab === 'applications' ? <FileCheck className="w-12 h-12 text-gray-300 mx-auto mb-4" /> : <Clock className="w-12 h-12 text-gray-300 mx-auto mb-4" />}
                <p className="text-gray-500">No {activeTab === 'applications' ? 'applications' : 'financing requests'} found.</p>
              </div>
            ) : (
                ...
                <thead className="table-header">
                  {activeTab === 'applications' ? (
                    ...
                    <tr>
                      ...
                      <th className="table-cell text-center">Workflow Step</th>
                      <th className="table-cell text-left">Actions</th>
                    </tr>
                  )}
                </thead>
                <tbody>
                  {activeTab === 'applications' ? applications.map((app) => (
                    <tr key={app._id} className="table-row">
                      ...
                      <td className="table-cell">${app?.requestedAmount?.toLocaleString()}</td>
                    ...
                    <tr key={req._id} className="table-row">
                      ...
                      <td className="table-cell text-center">
                        <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">
                          {req.workflowStep?.replace(/_/g, ' ') || 'PENDING'}
                        </span>
                      </td>
                      <td className="table-cell">
                        <button
                          onClick={() => navigate(`/admin/application/${req.pspId?._id}`)}
                          className="flex items-center gap-2 px-4 py-2 text-brand-purple hover:bg-brand-purple hover:text-white rounded-lg transition-colors font-medium"
                        >
                          <Eye className="w-4 h-4" />
                          View PSP
