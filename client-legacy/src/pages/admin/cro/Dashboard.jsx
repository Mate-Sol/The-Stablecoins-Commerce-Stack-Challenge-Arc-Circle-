  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [appsResponse, statsResponse] = await Promise.all([
        adminAPI.getApplications({ search, page, limit }),
        adminAPI.getStats()
      ]);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="stats-card">
              <span className="stats-label">Pending PSP Applications</span>
              <span className="stats-value text-status-warning">{stats?.pendingApplications}</span>
            </div>
            <div className="stats-card">
              <span className="stats-label">Active Liquidity Facilities</span>
          ...
          <div className="table-container mb-8">
            <div className="px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <h2 className="text-lg font-semibold whitespace-nowrap">Pending Applications</h2>
              ...
              <tbody>
                {applications.map((app) => (
                  <tr key={app.id} className="table-row">
                    ...
                    <td className="table-cell">${(app?.requestedAmount  || 0).toLocaleString({
                      style: 'currency',
                      currency: 'USD'
                    })}</td>
